...
class ProductController extends Controller
{
   ...
   /**
    ...
    {
        // Use getRelation() to bypass the model column/relationship naming collision
        $categoryModel = $product->getRelation('category');
        $brandModel = $product->getRelation('brand');
        ...
        return [
            ...
            // Extract from relation model if it exists, otherwise fallback to the string column
            'category' => $categoryModel ? $categoryModel->category : $product->category,
            'brand' => $brandModel ? $brandModel->name : $product->brand,
            
            'wishlist' => in_array($product->id, $wishlistProductIds),
            'stock' => $product->stock ? 'on' : null,
        ];
    }
    ...
    private function formatDetailProduct(Product $product, array $wishlistProductIds)
    {
        ...
        $data['category'] = $categoryModel ? $categoryModel->category : $product->category;
        $data['brand'] = $brandModel ? $brandModel->name : $product->brand;
